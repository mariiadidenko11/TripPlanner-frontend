...
function _normalizePlace(p) {
    return {
        ...
        is_favourite: p.isFavourite ?? false,
...
function _costVal(c) {
    return c['moneyСost'] ?? c.moneyCost ?? c.cost ?? 0;
}
...
function _normalizeStats(s, tripsStats = []) {
    ...
    // GetCost() already returns percentages (0-100) per type.
    const costRaw = s.costForStatistic ?? [];
    const cost_by_type = costRaw.map(c => {
        const percent = _costVal(c);
        return {
            type: c.type ?? '',
            percent,
            amount: Math.round((percent / 100) * (s.cost ?? 0)),
        };
    });
