...
function getDB() {
    ...
    try {
        ...
        // Robust consistency checks and initialization
        ...
        ['tasks', 'places', 'bookings', 'notes', 'photos'].forEach(key => {
            if (!db[key] || typeof db[key] !== 'object') {
                db[key] = INITIAL_DATA[key] || {};
            }
        });

        // MIGRATION: always sync trip covers from INITIAL_DATA
        db.trips = db.trips.map(trip => {
            const fresh = INITIAL_DATA.trips.find(t => t.id === trip.id);
            return fresh ? { ...trip, cover: fresh.cover } : trip;
        });

        // MIGRATION: if expenses missing or any trip has no expenses, restore from INITIAL_DATA
        if (!db.expenses || typeof db.expenses !== 'object') {
            db.expenses = INITIAL_DATA.expenses || {};
            localStorage.setItem(STORAGE_KEY, JSON.stringify(db));
        } else {
            // Fill in any trips that are missing expenses from INITIAL_DATA
            let migrated = false;
...
export async function login({ email, password }) {
    ...
    if (authData && password) {
        // In real app, we would verify hash. Here we just return the linked user.
        return {
...
export async function register({ firstname, lastname, email, password }) {
    ...
    db.user = newUser; // Set as current mock user
...
export async function updateUser(fields) {
    const db = getDB();
    // fields should only contain firstname, lastname, description
...
// -----------------------------------------------------------------------------
// PHOTO MAPPING HELPER
// -----------------------------------------------------------------------------

export function getPhotoForLocation(location = '') {
    const loc = (location || '').toLowerCase();
    if (loc.includes('santorini') || loc.includes('санторіні')) return 'https://images.unsplash.com/photo-1570077188670-e3a8d69ac5ff?q=80&w=1000&auto=format&fit=crop';
    if (loc.includes('kyiv') || loc.includes('київ')) return 'https://images.unsplash.com/photo-1561536340-025593399435?q=80&w=1000&auto=format&fit=crop';
    if (loc.includes('lviv') || loc.includes('львів')) return 'https://images.unsplash.com/photo-1548777123-e216912df7d8?q=80&w=1000&auto=format&fit=crop';
    if (loc.includes('paris') || loc.includes('париж')) return 'https://images.unsplash.com/photo-1502602898657-3e91760cbb34?q=80&w=1000&auto=format&fit=crop';
    if (loc.includes('rome') || loc.includes('рим')) return 'https://images.unsplash.com/photo-1552832230-c0197dd311b5?q=80&w=1000&auto=format&fit=crop';
    if (loc.includes('tokyo') || loc.includes('токіо')) return 'https://images.unsplash.com/photo-1503899036084-c55cdd92da26?q=80&w=1000&auto=format&fit=crop';
    if (loc.includes('iceland') || loc.includes('ісландія')) return 'https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?q=80&w=1000&auto=format&fit=crop';
    if (loc.includes('new york') || loc.includes('нью-йорк')) return 'https://images.unsplash.com/photo-1496442226666-8d4d0e62e6e9?q=80&w=1000&auto=format&fit=crop';
...
function _enhanceTrip(t, db) {
    ...
    // fact_money = sum of all expense entries (single source of truth)
    ...
    return {
        ...
        cover: t.cover || getPhotoForLocation(t.location || location),
        tasks_done,
        tasks_total,
        fact_money,
    };
...
export async function getTrips({ status = 'all' } = {}) {
    const db = getDB();
    // If trips array is empty, try to restore from INITIAL_DATA to ensure visibility
...
export async function createCity(cityData) {
    ...
    const trip = db.trips.find(t => t.id === cityData.trips_id);
    if (trip && !trip.cover) {
        trip.cover = getPhotoForLocation(newCity.name);
    }
...
function _mapTask(t) {
    return {
        ...t,
        value: t.value || t.text, // compatibility mapping
        check: t.check ?? t.done   // compatibility mapping
    };
}
...
function _mapPlace(p) {
    return {
        ...p,
        favourite: p.favourite ?? p.is_favourite // compatibility mapping
    };
}
...
function _mapPhoto(p) {
    return {
        ...p,
        src: p.src || p.url  // compatibility
    };
}

export async function getPhotos(tripId) {
    const db = getDB();
    return (db.photos[tripId] || []).map(_mapPhoto);
}

export async function uploadPhotos(tripId, files) {
    const db = getDB();
    if (!db.photos[tripId]) db.photos[tripId] = [];
    const newPhotos = Array.from(files).map((f, i) => ({
        id: 'ph' + Date.now() + i,
        url: URL.createObjectURL(f),
        created_at: new Date().toISOString()
    }));
    db.photos[tripId].push(...newPhotos);
    saveDB(db);
    return newPhotos.map(_mapPhoto);
}


export async function deletePhoto(tripId, photoId) {
    const db = getDB();
    if (db.photos[tripId]) {
        db.photos[tripId] = db.photos[tripId].filter(p => p.id !== photoId);
        saveDB(db);
    }
    return true;
}

export async function restorePhoto(tripId, photoId) { return true; }
