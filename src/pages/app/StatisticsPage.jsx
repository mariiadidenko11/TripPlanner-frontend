...
// Красива округлена «стеля» для осі Y на основі максимуму
...
function SpendingTrend({ monthly }) {
    ...
    const data = (monthly || []).map(x => ({
        label: MONTHS_SHORT[x.m] + (new Set((monthly || []).map(o => o.y)).size > 1 ? ` '${String(x.y).slice(2)}` : ''),
        val: x.total,
    }));

    if (data.length === 0) {
        return (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: 180, gap: 10, textAlign: 'center' }}>
                <span style={{ fontSize: 30 }}>📈</span>
                <p style={{ fontWeight: 600, color: 'var(--ink-deep)', fontSize: 14 }}>Немає даних для відображення</p>
                <p style={{ color: 'var(--slate)', fontSize: 12.5, maxWidth: 300 }}>Додайте витрати до поїздок із датами — і тут зʼявиться динаміка витрат по місяцях.</p>
            </div>
        );
    }

    const W = 560, H = 200, pad = { t: 18, b: 38, l: 54, r: 18 };
    const plotW = W - pad.l - pad.r;
    const plotH = H - pad.t - pad.b;
    const maxVal = niceCeil(Math.max(...data.map(d => d.val)));
    const yTicks = [0, 0.25, 0.5, 0.75, 1].map(f => Math.round(maxVal * f));

    const n = data.length;
    const xFor = (i) => n === 1 ? pad.l + plotW / 2 : pad.l + (i / (n - 1)) * plotW;
    const yFor = (v) => pad.t + plotH - (v / maxVal) * plotH;
    const pts = data.map((d, i) => [xFor(i), yFor(d.val)]);

    const linePath = pts.map((p, i) => (i === 0 ? `M${p[0]},${p[1]}` : `C${(pts[i - 1][0] + p[0]) / 2},${pts[i - 1][1]} ${(pts[i - 1][0] + p[0]) / 2},${p[1]} ${p[0]},${p[1]}`)).join(' ');
    const areaPath = n > 1 ? linePath + ` L${pts[n - 1][0]},${pad.t + plotH} L${pts[0][0]},${pad.t + plotH} Z` : '';
            ...
            <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', overflow: 'visible' }}>
                <defs>
                    <linearGradient id="trendArea" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#40B3E0" stopOpacity="0.20" />
                ...
                {yTicks.map((v, i) => {
                    ...
                    return (
                        <g key={i}>
                            <line x1={pad.l} y1={y} x2={W - pad.r} y2={y} stroke="rgba(16,24,40,0.06)" strokeWidth="1" strokeDasharray={i === 0 ? '0' : '3 5'} />
                            <text x={pad.l - 10} y={y + 3.5} textAnchor="end" fontSize="9.5" fontWeight="500" fill="#94a3b8">{fmtMoney(v)}</text>
                        </g>
                    );
                })}
                {n > 1 && <path d={areaPath} fill="url(#trendArea)" style={{ animation: 'trendFade 0.7s ease both' }} />}
                {n > 1 && <path d={linePath} fill="none" stroke="#40B3E0" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ strokeDasharray: 1200, strokeDashoffset: 1200, animation: 'trendDraw 0.9s cubic-bezier(0.16,1,0.3,1) forwards' }} />}
                {/* точки + зони ховера */}
                {pts.map(([x, y], i) => {
                    const isHover = hover === i;
                    return (
                        <g key={i} onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(null)} style={{ cursor: 'default' }}>
                            <rect x={x - (plotW / n) / 2} y={pad.t} width={plotW / n} height={plotH} fill="transparent" />
                            {isHover && <line x1={x} y1={pad.t} x2={x} y2={pad.t + plotH} stroke="#40B3E0" strokeWidth="1" strokeDasharray="3 3" opacity="0.5" />}
                            <circle cx={x} cy={y} r={isHover ? 5.5 : 3.5} fill="#fff" stroke="#40B3E0" strokeWidth={isHover ? 3 : 2} style={{ transition: 'r 0.15s, stroke-width 0.15s' }} />
                            {isHover && (
                                <text x={x} y={y - 12} textAnchor={i === 0 ? 'start' : i === n - 1 ? 'end' : 'middle'} fontSize="11" fontWeight="700" fill="var(--ink-deep)">{fmtMoney(data[i].val)}</text>
                            )}
                            <text x={x} y={H - pad.b + 16} textAnchor="middle" fontSize="9.5" fontWeight="500" fill={isHover ? 'var(--ink-deep)' : '#94a3b8'}>{data[i].label}</text>
                        </g>
                    );
                })}
            </svg>
...
function BudgetDonut({ trips, costByType, totalSpent }) {
    // Дані строго з backend: costForStatistic -> cost_by_type [{ type, percent, amount }]
    const palette = ['#40B3E0', '#f59e0b', '#6366f1', '#22c55e', '#ec4899', '#94a3b8'];
    const source = (costByType || []).filter(c => (c.amount || 0) > 0);
    const total = Number(totalSpent || source.reduce((s, c) => s + (c.amount || 0), 0));
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, flex: 1, minWidth: 200 }}>
                {hasData ? items.map((it, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ color: 'var(--ink-deep)', fontSize: 12, flex: 1, fontWeight: 500 }}>{it.label}</span>
                        <span style={{ color: 'var(--slate)', fontSize: 12 }}>${it.amount.toLocaleString()}</span>
                        <strong style={{ color: 'var(--ink-deep)', fontSize: 12, fontWeight: 700, minWidth: 42, textAlign: 'right' }}>{fmtPct(it.pct)}%</strong>
                    </div>
                )) : (
                    <span style={{ color: 'var(--slate)', fontSize: 12, fontStyle: 'italic' }}>Немає даних про витрати</span>
                )}
            </div>
        </div>
    );
...
export default function StatisticsPage() {
    ...
    const { data: summary, loading, error } = useApi(() => stats.summary(period), [period]);
    // Реальні подорожі з датами (endpoint /api/statistic/trips дат не віддає) — для помісячного тренду
    const { data: realTrips } = useApi(() => getTrips(), []);

    const s = summary || {};
    const trips = s.trips_comparison || [];

    // Зіставляємо рядок статистики з реальною подорожжю за назвою → беремо дати
    const monthlySpending = (() => {
        const byName = {};
        (realTrips || []).forEach(rt => { byName[(rt.name || '').trim()] = rt; });
        // сума витрат по місяцях (рахуємо за датою початку поїздки)
        const sums = {}; // 'YYYY-M' -> { y, m, total }
        trips.forEach(t => {
            const real = byName[(t.title || t.name || '').trim()];
            const start = real?.start_at;
            const spent = Number(t.spent || 0);
            if (!start || spent <= 0) return;
            const d = new Date(start);
            if (isNaN(d)) return;
            const key = `${d.getFullYear()}-${d.getMonth()}`;
            (sums[key] ||= { y: d.getFullYear(), m: d.getMonth(), total: 0 }).total += spent;
        });
        return Object.values(sums).sort((a, b) => a.y - b.y || a.m - b.m);
    })();
    ...
    if (error) return (
        <div className="container page" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 300 }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, textAlign: 'center' }}>
                ...
                <p style={{ color: 'var(--ink-deep)', fontWeight: 600, fontSize: 15 }}>Не вдалося завантажити статистику</p>
                <p style={{ color: 'var(--slate)', fontSize: 13 }}>
                    {error.status === 401 ? 'Потрібно увійти в акаунт.' : 'Перевірте з’єднання та спробуйте ще раз.'}
                </p>
            </div>
        </div>
    );

    // Порожній стан: на бекенді ще немає жодної подорожі
    const isEmpty = (s.total_trips || 0) === 0 && trips.length === 0;
    if (isEmpty) return (
        <div className="container page" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 300 }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, textAlign: 'center' }}>
                <span style={{ fontSize: 32 }}>📊</span>
                <p style={{ color: 'var(--ink-deep)', fontWeight: 600, fontSize: 15 }}>Поки що немає даних для аналітики</p>
                <p style={{ color: 'var(--slate)', fontSize: 13 }}>Створіть подорож, додайте витрати й завдання — статистика з’явиться тут.</p>
            </div>
        </div>
    );
