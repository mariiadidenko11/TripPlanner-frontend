...
// УВАГА: типи бронювань зберігаються в БД backend у таблиці `reservation_type` (id, value).
// Окремого GET-endpoint для типів НЕМАЄ. При створенні backend шукає тип по `value`
// (SELECT id FROM reservation_type WHERE value=@value). Тому `key` мусить ДОРІВНЮВАТИ
// значенню `value` у БД — інакше тип не знайдеться (type_id=0 → помилка створення).
//
// ВАЖЛИВО: щоб усі 6 категорій працювали, у таблиці reservation_type мають бути рядки:
//   'Проживання', 'Транспорт', 'Харчування', 'Дозвілля', 'Шопінг', 'Інше'
// (підтверджені раніше зі статистики: 'Готель', 'Літак', 'Інше' — їх теж підхоплюємо як синоніми).
const CATS = [
    { id: 1, key: 'Проживання', label: 'Проживання', icon: '🏨', color: '#8B5CF6' },
    { id: 2, key: 'Транспорт', label: 'Транспорт', icon: '✈️', color: '#0EA5E9' },
    { id: 3, key: 'Харчування', label: 'Харчування', icon: '🍽️', color: '#F97316' },
    { id: 4, key: 'Дозвілля', label: 'Дозвілля', icon: '🎭', color: '#EC4899' },
    { id: 5, key: 'Шопінг', label: 'Шопінг', icon: '🛍️', color: '#EF4444' },
    { id: 6, key: 'Інше', label: 'Інше', icon: '📌', color: '#9CA3AF' },
];

// Синоніми значень з БД → наш ключ (на випадок, якщо в таблиці інші формулювання).
const CAT_SYNONYMS = {
    'Готель': 'Проживання',
    'Літак': 'Транспорт',
...
// Повертає оформлення для значення типу з backend (з урахуванням синонімів).
// Якщо тип невідомий — показуємо його як є з нейтральним виглядом.
function catFor(typeValue) {
    const normalized = CAT_SYNONYMS[typeValue] || typeValue;
    return CATS.find(c => c.key === normalized)
        || { key: typeValue, label: typeValue || 'Бронювання', icon: '📌', color: '#9CA3AF' };
}
...
export default function BookingPage() {
    ...
    // Backend GET повертає ВСІ бронювання трипа (фільтра за типом немає) — фільтруємо на клієнті
    const bookings = catFilter ? allBookings.filter(b => catFor(b.type).key === catFilter) : allBookings;
    ...
    const submit = async () => {
        ...
        try {
            // Назва: пріоритет імені, потім адреса, потім назва категорії
            const catLabel = catFor(selTypeId).label;
            const finalName = name.trim() || address.trim() || catLabel;

            await createBooking(tripId, {
                name: finalName,
                type: selTypeId,            // надсилаємо ЗНАЧЕННЯ типу (value з БД), а не key
