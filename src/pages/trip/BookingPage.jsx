...
import { getBookings, createBooking, deleteBooking, restoreBooking } from '@/api/api';
...
export default function BookingPage() {
    ...
    const submit = async () => {
        if (!selTypeId || !address.trim() || !startAt || !endAt) { 
            toast('Заповніть обов\'язкові поля: категорія, місце, початок та кінець', 'warning'); 
            return; 
        }
        try {
            ...
            setAddress(''); setCost(''); setNote(''); setSelTypeId(null); 
        ...
        <div className="booking-container">
            ...
            <div className="booking-filters">
                ...
                {CATS.map(c => (
                    <span 
                        ...
                        className={`booking-chip ${catFilter === c.id ? 'booking-chip--active' : ''}`}
                    ...
                    <div className="booking-form-cats">
                        {CATS.map(c => (
                            <button 
                                key={c.id} 
                                type="button" 
                                onClick={() => setSelTypeId(c.id)}
                                className={`booking-form-cat-btn ${selTypeId === c.id ? 'booking-form-cat-btn--active' : ''}`}
                                style={selTypeId === c.id ? { borderColor: c.color, backgroundColor: `${c.color}15` } : {}}
                            >
                                <span className="cat-icon">{c.icon}</span>
                                <span className="cat-label" style={selTypeId === c.id ? { color: c.color } : {}}>{c.label}</span>
                            </button>
                        ))}
                    </div>
                    <div className="booking-form-fields">
                        <input value={address} onChange={e => setAddress(e.target.value)} placeholder="Місце / Адреса *" className="booking-input" />
            ...
            {bookings.length === 0 && !loading ? (
                ...
                <div className="booking-list">
                    {bookings.map(b => {
                        const cat = CATS.find(c => c.id === b.type_id) || CATS[5];
                        ...
                        return (
                            <div key={b.id} className="booking-item">
                                ...
                                <div className="booking-item-content">
                                    ...
                                    <span className="booking-item-title">{b.address}</span>
                                </div>
