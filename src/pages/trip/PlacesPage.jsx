...
export default function PlacesPage() {
    ...
    const add = async () => {
        ...
        try {
            await createPlace(tripId, {
                ...
                is_favourite: false, favourite: false
            });
            setNewName('');
            refetch();
    ...
    const toggleFav = async (place) => {
        const isFav = !(place.is_favourite || place.favourite);
        try {
            await updatePlace(tripId, place.id, { is_favourite: isFav, favourite: isFav });
            refetch();
        } catch (err) { toast(err.message, 'error'); }
    };
    ...
    return (
        <div className="places-container">
                        ...
                        <div key={p.id} className={`place-card ${(p.visited || p.check) ? 'place-card--visited' : ''}`}>
                            <div className="place-card__main">
                                ...
                                <div className="place-actions">
                                    <button onClick={() => toggleFav(p)} className={`fav-btn ${(p.is_favourite || p.favourite) ? 'fav-btn--active' : ''}`} title="Улюблене">★</button>
            ...
            <style>{`
                ...
                .fav-btn { background: none; border: none; cursor: pointer; color: var(--hairline); font-size: 18px; transition: color 0.15s; line-height: 1; }
                .fav-btn--active { color: #FFBA00; }
