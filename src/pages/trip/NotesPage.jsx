...
// DB: public.trip_notes stores one field 'value' (text) + 'check' (bool).
// UI shows title + body for convenience.
// We encode them as: "title\nbody" in the 'value' field.
// These helpers do the conversion in both directions.
...
export default function NotesPage() {
    ...
    // -------------------------------------------------------------------------
    // CRUD — all sends use DB field names: value, check, trips_id
    // -------------------------------------------------------------------------
    ...
    const saveNote = async () => {
        ...
        try {
            await createNote(tripId, {
                value: encodeNoteValue(composerTitle, composerBody), // DB: public.trip_notes.value
                check: false,                                         // DB: public.trip_notes.check
                trips_id: tripId                                      // DB: public.trip_notes.trips_id
            });
            setComposerTitle('');
            setComposerBody('');
    ...
    const saveEdit = async () => {
        try {
            await updateNote(tripId, editId, {
                value: encodeNoteValue(editTitle, editBody) // DB: public.trip_notes.value
            });
            setEditId(null);
            refetch();
            toast('Нотатку збережено', 'success');
        ...
        <div className="notes-container">
            ...
            {notes.length === 0 && !loading ? (
                ...
                <div className="notes-list">
                    {notes.map(note => {
                        // Decode value → title + body for display
