...
export default function OverviewPage() {
    const { trip, tripId, refetchTrip } = useOutletContext();
    ...
    const saveEdit = async () => {
        ...
        try {
            await updateTrip(tripId, {
                ...
                description: trip?.description || '', // Зберігаємо поточний опис
            });
    ...
    // Функции для управления описанием
    ...
    return (
        <div className="ov-grid">
            ...
            <div className="ov-card ov-card--full ov-card--relative">
                <h3 className="ov-card__title">
                    <span>📝</span> Опис подорожі
                </h3>
                
                <div className="ov-desc-wrapper">
                    {!isEditingDesc ? (
                        <>
                            <div className="ov-note-content">
                                {trip?.description || <span className="ov-empty">Опис поки що відсутній...</span>}
                            </div>
                            <button className="ov-edit-btn" onClick={handleStartEditDesc} title="Редагувати">
                                <IconEdit />
                            </button>
                        </>
                    ) : (
                        <div className="ov-edit-note-standalone">
                            <textarea
                                ...
                                autoFocus
                            />
                            <div className="ov-edit-actions" style={{ borderTop: 'none', padding: '12px 0 0' }}>
                                <button className="ov-edit-cancel" onClick={handleCancelDesc} disabled={descSaving}>
                                    <IconX /> Скасувати
                                </button>
                                <button className="ov-edit-save" onClick={handleSaveDesc} disabled={descSaving}>
                                    <IconCheck /> {descSaving ? 'Збереження...' : 'Зберегти'}
                                </button>
                            </div>
                        </div>
                    )}
            ...
            <style>{`
                ...
                .ov-card--details, .ov-card--relative { position: relative; min-height: 100%; box-sizing: border-box; }
                .ov-card__title {
                ...
                .ov-note-box {
                    ...
                    border-left: 3px solid #40B3E0;
                }
                .ov-note-header {
                    display: flex; align-items: center; gap: 6px;
                    margin-bottom: 8px;
                }
                .ov-note-icon { font-size: 14px; }
                .ov-note-label {
                ...
                .ov-edit-note-standalone {
                    display: flex;
                    flex-direction: column;
                }
                .ov-edit-note-header { display: flex; align-items: center; gap: 6px; }
                .ov-edit-textarea {
                    width: 100%;
                    padding: 10px 12px;
                    border: 1.5px solid var(--hairline);
